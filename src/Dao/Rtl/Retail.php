...
class Retail extends Table{
    ...
    {
        $sqlstr = "select IFNULL(z.anoncartid, 0) as anoncartid, z.invPrdId, z.nombre, z.descripcion, z.precio as cartPrc, z.cantidad, sum(z.cartCtd) as cartCtd , z.cantidad - sum(z.cartCtd) as StockDisp
        from (
        select d.anoncartid, a.invPrdId, a.nombre, a.descripcion, a.precio, b.cantidad,  ifnull(d.cartCtd, 0) as cartCtd from
            celular a
            left join inventario b on a.invPrdId = b.invPrdId
            left join carretilla_anon d on a.invPrdId = d.invPrdId and d.cartIat >= now()
        where a.estado='ACT'
         ) as z
        group by  z.anoncartid, z.invPrdId, z.nombre, z.descripcion, z.precio, z.cantidad
        ;";
